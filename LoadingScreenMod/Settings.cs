...
namespace LoadingScreenMod
{
	public class Settings
	{
		...
		private UIHelper CreateGroup(UIHelperBase parent, string name, string tooltip = null)
		{
			...
			UIPanel obj2 = obj.self as UIPanel;
			RectOffset rectOffset = obj2?.autoLayoutPadding;
			if (rectOffset != null)
			{
				rectOffset.bottom /= 2;
			}
			UIPanel uIPanel = obj2?.parent as UIPanel;
			RectOffset rectOffset2 = uIPanel?.autoLayoutPadding;
			if (rectOffset2 != null)
			{
		...
		private void TextField(UIHelper group, string text, OnTextChanged action)
		{
			try
			{
				...
				UILabel uILabel = parent?.Find<UILabel>("Label");
				if (uILabel != null)
				{
