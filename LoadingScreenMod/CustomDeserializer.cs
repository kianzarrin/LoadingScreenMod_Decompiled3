...
namespace LoadingScreenMod
{
	internal sealed class CustomDeserializer : Instance<CustomDeserializer>
	{
		...
		private static void ReadOutUITextureAtlas(Package p, PackageReader r)
		{
			...
			for (int offset = 0; offset < name; offset++)
			{
				r.ReadInt32();
				r.ReadInt32();
				r.ReadInt32();
		...
		{
			string servicePrefixes = r.ReadString();
			NetInfo subServicePrefixes = Get<NetInfo>(servicePrefixes);
			if (recordUsed && !string.IsNullOrEmpty(servicePrefixes))
			{
				AddRef(subServicePrefixes, servicePrefixes, CustomAssetMetaData.Type.Road);
			}
			BuildingInfo.PathInfo dlcs = new BuildingInfo.PathInfo();
			dlcs.m_netInfo = subServicePrefixes;
			dlcs.m_nodes = r.ReadVector3Array();
			dlcs.m_curveTargets = r.ReadVector3Array();
			dlcs.m_invertSegments = r.ReadBoolean();
			dlcs.m_maxSnapDistance = r.ReadSingle();
			if (p.version >= 5)
			{
				dlcs.m_forbidLaneConnection = r.ReadBooleanArray();
				dlcs.m_trafficLights = (BuildingInfo.TrafficLights[])(object)r.ReadInt32Array();
				dlcs.m_yieldSigns = r.ReadBooleanArray();
			}
			return dlcs;
		}
		...
		{
			PropInfo.SpecialPlace i = default(PropInfo.SpecialPlace);
			i.m_specialFlags = (CitizenInstance.Flags)r.ReadInt32();
			i.m_position = r.ReadVector3();
			i.m_direction = r.ReadVector3();
			return i;
		}
		...
		private static object ReadTreeInfoVariation(Package p, PackageReader r)
		{
			...
			string text = p.packageName + "." + i;
			TreeInfo tree = null;
			if (text == Instance<AssetLoader>.instance.Current.fullName)
			{
				Util.DebugPrint("Warning:", text, "wants to be a tree variation for itself");
			}
			else
			{
				tree = Get<TreeInfo>(p, text, i, tryName: false);
			}
		...
		{
			int i = r.ReadInt32();
			Dictionary<string, byte[]> dictionary = new Dictionary<string, byte[]>(i);
			for (int j = 0; j < i; j++)
			{
				dictionary[r.ReadString()] = r.ReadBytes(r.ReadInt32());
			}
			return dictionary;
		}
		...
		{
			PropInfo.ParkingSpace i = default(PropInfo.ParkingSpace);
			i.m_position = r.ReadVector3();
			i.m_direction = r.ReadVector3();
			i.m_size = r.ReadVector3();
			return i;
		}

		private static object ReadDisasterPropertiesDisasterSettings(PackageReader r)
		{
			DisasterProperties.DisasterSettings i = default(DisasterProperties.DisasterSettings);
			i.m_disasterName = r.ReadString();
			i.m_randomProbability = r.ReadInt32();
			return i;
		}
		...
		{
			int i = r.ReadInt32();
			NetLaneProps ret = ScriptableObject.CreateInstance<NetLaneProps>();
			ret.m_props = new NetLaneProps.Prop[i];
			for (int j = 0; j < i; j++)
			{
				ret.m_props[j] = ReadNetLaneProp(p, r);
			}
			return ret;
		}
		...
		private void AddRef(PrefabInfo info, string fullName, CustomAssetMetaData.Type type)
		{
			if (info == null)
			{
				...
				{
					Package.Asset from = FindContainer();
					if (from != null)
					{
						Instance<Reports>.instance.AddReference(from, fullName, type);
					}
				}
			}
			else
			{
				...
				if (!string.IsNullOrEmpty(to) && asset != null)
				{
					string packageName = asset.package.packageName;
					int e = to.IndexOf('.');
					string fullName2;
					if (e >= 0 && (e != packageName.Length || !to.StartsWith(packageName)) && (fullName2 = FindMain(to)) != null)
					{
						Instance<Reports>.instance.AddReference(asset, fullName2, type);
					}
				}
			}
		}
		...
		private static string ResolveCustomAssetName(string fullName)
		{
			...
			{
				Package.Asset[] from = Assets;
				for (int to = 0; to < from.Length; to++)
				{
					if (fullName == from[to].name)
					{
						return from[to].package.packageName + "." + fullName;
					}
				}
			}
			return fullName;
		...
		internal void AddPackage(Package p)
		{
			...
			else if (packages.TryGetValue(packageName, out d))
			{
				List<Package> list = d as List<Package>;
				if (list != null)
				{
					list.Add(p);
					return;
		...
		{
			int num = r.ReadInt32();
			byte[] e = new byte[num << 2];
			int i = 0;
			int num2 = 0;
			for (; i < num; i++)
			{
				e[num2++] = (byte)(r.ReadSingle() * 255f);
				e[num2++] = (byte)(r.ReadSingle() * 255f);
				e[num2++] = (byte)(r.ReadSingle() * 255f);
				e[num2++] = (byte)(r.ReadSingle() * 255f);
			}
		...
		private void ProcessAtlas(AtlasObj ao)
		{
			...
			for (int i = 0; i < sprites.Count; i++)
			{
				...
				int num4 = Mathf.FloorToInt((float)e * spriteInfo3.region.width);
			...
			if (spriteInfo == null || num > 5 || num2 > 1 || padding > 2 || e > 512 || (spriteInfo2 == null && num2 > 0))
			{
			...
			if (spriteInfo2 != null)
			{
				CopySprite(array, e, Mathf.FloorToInt((float)e * spriteInfo2.region.x), Mathf.FloorToInt((float)height * spriteInfo2.region.y), array2, 512, 0, 0, 75264, 1);
			...
			for (int j = 0; j < sprites.Count; j++)
			{
				...
				if (spriteInfo4 != spriteInfo2 && spriteInfo4.name.StartsWith(spriteInfo.name))
				{
					...
					{
						CopyHalf(array, e, Mathf.FloorToInt((float)e * spriteInfo4.region.x), Mathf.FloorToInt((float)height * spriteInfo4.region.y), array2, 512, num7, num8, 66, 66);
					...
					{
						CopySprite(array, e, Mathf.FloorToInt((float)e * spriteInfo4.region.x), Mathf.FloorToInt((float)height * spriteInfo4.region.y), array2, 512, num7, num8, 109, 100);
