...
namespace LoadingScreenMod
{
	public sealed class LoadingScreen : DetourUtility<LoadingScreen>
	{
		...
		internal override void Dispose()
		{
			...
			Text[] pfMegas = texts;
			for (int wsMegas = 0; wsMegas < pfMegas.Length; wsMegas++)
			{
				pfMegas[wsMegas].Dispose();
			}
		...
		{
			LoadingScreen fptr1 = Instance<LoadingScreen>.instance;
			if (fptr1.imageMaterial != null)
			{
				UnityEngine.Object.Destroy(fptr1.imageMaterial);
			}
			fptr1.imageMesh = mesh;
			fptr1.imageMaterial = new Material(material);
			fptr1.imageScale = scale;
			fptr1.imageLoaded = true;
		}
		...
		public void SetFont()
		{
			try
			{
				...
				List<Text> state = new List<Text>(5);
				state.Add(new Text(new Vector3(-1.3f, 0.7f, 10f), new DualProfilerSource(L10n.Get(4), 36)));
				state.Add(new Text(new Vector3(-0.33f, -0.52f, 10f), new SimpleProfilerSource(L10n.Get(5), Singleton<LoadingManager>.instance.m_loadingProfilerMain)));
				state.Add(new Text(new Vector3(-0.33f, -0.62f, 10f), new SimpleProfilerSource(L10n.Get(6), Singleton<LoadingManager>.instance.m_loadingProfilerSimulation)));
				state.Add(new Text(new Vector3(-0.12f, 0.7f, 10f), new LineSource(L10n.Get(7), 2, LevelLoader.AssetsActive)));
				state.Add(new Text(new Vector3(-0.08f, 0.43f, 10f), new TimeSource(), 1.4f));
				if (Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor)
				{
					state.Add(new Text(new Vector3(-0.08f, 0.32f, 10f), new MemorySource(), 1.4f));
				}
				texts = state.ToArray();
				fontLoaded = uifont != null;
			}
			catch (Exception sitePtr)
			{
				Util.DebugPrint("Font setup failed");
				Debug.LogException(sitePtr);
			}
		}

		private void RequestCharacterInfo()
		{
			UIDynamicFont sitePtr = uifont as UIDynamicFont;
			if (!(sitePtr == null) && UIFontManager.IsDirty(sitePtr))
			{
				sitePtr.AddCharacterRequest("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.-:", 1, FontStyle.Normal);
			}
		}
		...
		private void Update()
		{
			...
			loadingScreen.timer += Mathf.Min(0.125f, Time.deltaTime);
