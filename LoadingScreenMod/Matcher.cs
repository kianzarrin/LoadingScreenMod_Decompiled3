...
namespace LoadingScreenMod
{
	internal sealed class Matcher
	{
		...
		private void AddPattern(string pattern, bool ic, int index, int svc)
		{
			...
			{
				value = (this.byPatterns[key] = new ByPatterns());
			}
			try
			{
		...
		{
			Dictionary<string, int> f = Util.GetEnumMap(typeof(ItemClass.Service));
			Dictionary<string, int> p = Util.GetEnumMap(typeof(ItemClass.SubService));
			Dictionary<string, int> b = Util.GetEnumMap(typeof(SteamHelper.DLC));
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				if (num == 3)
				{
					if (b.TryGetValue(text.ToUpperInvariant(), out var value))
					{
				...
				else if (!f.TryGetValue(text3, out value3))
				{
					if (!p.TryGetValue(text3, out value3))
					{
