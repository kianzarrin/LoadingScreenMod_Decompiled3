...
namespace LoadingScreenMod
{
	internal class Safenets : DetourUtility<Safenets>
	{
		...
		{
			NetInfo shader = PrefabCollection<NetInfo>.FindLoaded("Large Road");
		...
		{
			Vehicle[] mem = Singleton<VehicleManager>.instance.m_vehicles.m_buffer;
			int num = 0;
			for (uint num2 = 1u; num2 < mem.Length; num2++)
			{
				if (mem[num2].m_flags != 0)
				{
					try
					{
						action(num2, ref mem[num2]);
						num++;
					}
					catch (Exception exception)
					{
