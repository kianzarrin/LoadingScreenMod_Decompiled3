		...
		public Coroutine LoadLevel(Package.Asset asset, string playerScene, string uiScene, SimulationMetaData ngs, bool forceEnvironmentReload = false)
		{
			...
			if (!loadingManager.m_currentlyLoading && !loadingManager.m_applicationQuitting)
			{
				...
				if (flag)
				{
					...
					Instance<LevelLoader>.instance.cityName = asset?.name ?? "NewGame";
					Profiling.Init();
		...
		public IEnumerator LoadLevelCoroutine(Package.Asset asset, string playerScene, string uiScene, SimulationMetaData ngs, bool forceEnvironmentReload)
		{
			...
			if (lm.m_loadedEnvironment == null)
			{
				...
				if (!string.IsNullOrEmpty(playerScene))
				{
					...
					AsyncOperation op3 = SceneManager.LoadSceneAsync(playerScene, LoadSceneMode.Single);
					while (!op3.isDone)
					{
						lm.SetSceneProgress(op3.progress * 0.01f);
						yield return null;
					}
					lm.m_loadingProfilerScenes.EndLoading();
				}
				...
				for (i = 0; i < levels.Length; i++)
				{
					...
					AsyncOperation op3 = SceneManager.LoadSceneAsync(key, LoadSceneMode.Additive);
					while (!op3.isDone)
					{
						lm.SetSceneProgress(currentProgress + op3.progress * (levels[i].Value - currentProgress));
						yield return null;
					}
					lm.m_loadingProfilerScenes.EndLoading();
				...
				if (!string.IsNullOrEmpty(key))
				{
					...
					while (!op3.isDone)
					{
						lm.SetSceneProgress(0.85f + op3.progress * 0.05f);
						if (optimizeThumbs)
						{
				...
				if (!string.IsNullOrEmpty(uiScene))
				{
					...
					AsyncOperation op3 = SceneManager.LoadSceneAsync(uiScene, LoadSceneMode.Additive);
					while (!op3.isDone)
					{
						lm.SetSceneProgress(0.9f + op3.progress * 0.08f);
						if (optimizeThumbs)
						{
							Instance<CustomDeserializer>.instance.ReceiveAvailable();
						}
			...
			else
			{
				...
				{
					lm.m_loadingProfilerScenes.BeginLoading(key);
					yield return SceneManager.LoadSceneAsync(key, LoadSceneMode.Additive);
					lm.m_loadingProfilerScenes.EndLoading();
				}
			}
